...
import { AddressZero, DeterministicDeployer, erc4337RuntimeVersion, SimpleAccountFactory__factory, sleep } from '@account-abstraction/utils'
import fs from 'fs'
import { HttpRpcClient, SimpleAccountAPI } from '@account-abstraction/sdk'
import { runBundler } from '../runBundler'
import { BundlerServer } from '../BundlerServer'
import { getNetworkProvider } from '../Config'
import { PaymasterParams } from '@account-abstraction/sdk/dist/src/PaymasterAPI'

const ENTRY_POINT = '0x4337084D9E255Ff0702461CF8895CE9E3b5Ff108'
const PAYMASTER = '0x17cf225bEFBdC683A48DB215305552B3897906F6'
...
class Runner {
  ...
  async init (deploymentSigner?: Signer): Promise<this> {
    ...
    let accountDeployer = await DeterministicDeployer.getAddress(new SimpleAccountFactory__factory(), 0, [this.entryPointAddress])
    ...
    if (!await dep.isContractDeployed(accountDeployer)) {
      ...
      const res = await dep1.deterministicDeploy(new SimpleAccountFactory__factory(), 0, [this.entryPointAddress])
      console.log(`dep1.deterministicDeploy at ${res}`)

      accountDeployer = accountDeployer = await DeterministicDeployer.getAddress(new SimpleAccountFactory__factory(), 0, [this.entryPointAddress])
      console.log(`accountDeployer at ${accountDeployer}`)
    }
    this.bundlerProvider = new HttpRpcClient(this.bundlerUrl, this.entryPointAddress, chainId)

    const paymasterAPI = {
      async getTemporaryPaymasterData (userOp: any): Promise<PaymasterParams | null> {
        const result = {
          paymaster: PAYMASTER,
          paymasterData: '0x',
        ...
        console.log('PaymasterAPI getTemporaryPaymasterData:', result)
        return result
      },
      async getPaymasterData (userOp: any) {
        const result = await this.getTemporaryPaymasterData(userOp)
        console.log('PaymasterAPI getPaymasterData:', result)
        return result
      }
    }
...
async function main (): Promise<void> {
  ...
  if (bal.lt(parseEther('2'))) {
    console.log('funding account to', parseEther('3'))
    await signer.sendTransaction({
      to: addr,
      value: parseEther('3').sub(bal)
